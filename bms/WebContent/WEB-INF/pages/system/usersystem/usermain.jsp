		...
		<div id="user">
			<div class="active">
					...
					<font class="book">
						<form action="user_page" method="post">
							...
							<input type="text" id="textbook" class="form-control"
								placeholder="请输入查找的用户" name="loginName" />
				...
				<table id="bookbox" border="1px solid red" cellpadding="0px"
					...
					<c:if test="${!empty bmuser_one}">
					<c:forEach items="${bmuser_one }" var="emp" varStatus="status">	
						<tr id="bookr2">
								<td>${status.count}</td>
								<td>${emp.loginName }</td>
								<td>${emp.name }</td>
								...
								<td><a href="bm_userdelete?bmuser.id=${emp.id}"
					...
					<c:if test="${!empty bmuser }">
						
						<c:forEach items="${bmuser }" var="emp" varStatus="status">
							<tr id="bookr2">
								<td>${status.count}</td>
								<td>${emp.loginName }</td>
								<td>${emp.name }</td>
						...
						<tr style="font-size: 20px; font-weight: bold;">
							<th colspan="2" style="text-align: center;"><a
								style="color: red;" href="usermain?page=1"><<首页>></a></th>
							<th colspan="2" style="text-align: center;"><c:if
									test="${page-1>=1 }">
									<a style="color: red;" href="usermain?page=${page-1 }">上一页</a>
								</c:if></th>
							<th colspan="2" style="text-align: center;"><a
								style="color: red;">当前第${page}页</a></th>
							<th colspan="2" style="text-align: center;"><c:if
									test="${maxpage>page }">
									<a style="color: red;" href="usermain?page=${page+1 }">下一页</a>
								</c:if></th>
							<th colspan="3" style="text-align: center;"><a
								style="color: red;" href="usermain?page=${maxpage}"><<尾页>></a></th>
