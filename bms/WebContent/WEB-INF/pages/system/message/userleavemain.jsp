...
<body>
	...
	<div class="right">
		<div id="user">
			<div class="active">
				<h1 id="titlebook">
					<li class="btn btn-info"><span style="font-size: 30px"
						class="glyphicon glyphicon-home">留言管理</span></li>
				</h1>
			...
			<c:forEach items="${messagelist}" var="mess" varStatus="st">
					<tr id="boxmess">
						<th>${st.count}</th>
						<th>${mess.leaveword}</th>
						<th>${mess.loginName}</th>
						<th>${mess.leavewordTime}</th>
						<th><a href="#">回复</a>|<a href="delmess?bmLeaveWord.id=${mess.id }">删除</a></th>
					</tr>
			</c:forEach>
				<h6 style="font-size: 15px; font-weight: bold;">
							<th style="text-align: center;"><a 
								href="messagex?page=1"><<首页>></a></th>
							<th style="text-align: center;"><c:if test="${page-1>=1 }">
									<a  href="messagex?page=${page-1 }">上一页</a>
								</c:if></th>
							<th style="text-align: center;"><a>当前第${page}页</a></th>
							<th style="text-align: center;"><c:if
									test="${maxpage>page }">
									<a  href="messagex?page=${page+1 }">下一页</a>
								</c:if></th>
							<th style="text-align: center;"><a 
								href="messagex?page=${maxpage}"><<尾页>></a></th>
						</h6>
			</c:if>
